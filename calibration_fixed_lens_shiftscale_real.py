# -*- coding: utf-8 -*-
"""
Created on Wed Mar 11 11:37:06 2020

@author: u6265553
"""

'''This script generates the inverse transformation function required 
for fixed lens shift-scale calibration to align the images to a reference 
...
imglist=glob.glob('F:/Arif/fixed_lens/fixed_lens_calibration/100EOS5D/*.jpg')
img1 = cv2.imread(imglist[30])
...
#cv2.imwrite('F:/Arif/fixed_lens/insect_latest/2020_03_19/trans_img/img.png',img1)  
...
del_y_pixel=np.mean(np.array([(centers1[1][0][0]-centers1[0][0][0]),(centers1[2][0][0]-centers1[1][0][0]),(centers1[3][0][0]-centers1[2][0][0])]))
#computing magnification
M=del_y_pixel/Del_Y_pixel
'''

#Magnification for fixed lens capture
M=1.7
pixelsize=4e-6
...
for i in range(num_of_img):
    ...
    #shift_x[i]=(1-scaling[i])*(4320/8)+(mu*(num_of_img-1-i)*del_d_pixel)/4
    #shift_y[i]=(1-scaling[i])*(2880/8)+(Lambda*(num_of_img-1-i)*del_d_pixel)/4
